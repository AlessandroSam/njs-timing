...
app.post('/ACTiming2/live', function(req, res, next) {
    sessionData = {
        ...
        isTimedRace: req.body.isTimedRace
    }
    
    if (!req.body.cars) return;
    req.body.cars.forEach((car, index, cars) => {
        let isRace = req.body.session == 2;
        let leadTime = (isRace) ? cars[0].totalTime : cars[0].bestLap;
        let gap = (isRace) ? car.totalTime : car.bestLap;
        if (!isRace || car.lapsCompleted === cars[0].lapsCompleted) {
            gap -= leadTime;
            car.gap = gap;
            car.isLapped = false;
        } else {
            car.isLapped = true;
            car.gap = (cars[0].lapsCompleted - car.lapsCompleted);
        }
        if (car.lapsCompleted === 0) car.gap = null;
    });
